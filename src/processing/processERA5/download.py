...
surface_vars = ['2m_dewpoint_temperature', '2m_temperature', 'convective_available_potential_energy',
                'convective_inhibition', 'k_index', 'precipitation', 'specific_rain_water_content', 'surface_pressure',
...
level_vars = ['relative_humidity', 'specific_humidity', 'temperature', 'u_component_of_wind', 'vertical_velocity']
...
def download_variable(var, levs):
...
if __name__ == "__main__":
    ...
    pool_single.map(download_variable, surface_vars, None)

    # Close the pool
    pool_single.close()
    pool_single.join()

    # Repeat above for multi-level variables
    pool_level = multiprocessing.Pool()
    pool_level.map(download_variable, surface_vars, levels)
    pool_level.close()
    pool_level.join()
